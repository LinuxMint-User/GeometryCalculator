...
import { ratToNumber, type Expr, type Sign } from './expr.js'

// ---------- Expr → 数值 ----------

/** 表达式数值求值（测试/筛根/回代用）；未定义符号返回 NaN */
export function evalFloat(e: Expr): number {
  switch (e.kind) {
    case 'num':
...
export function exprToFunction(e: Expr, varName: string): (x: number) => number {
  const go = (node: Expr): ((x: number) => number) => {
    switch (node.kind) {
      case 'num': {
      ...
      case 'sym':
        if (node.name === varName) return (x) => x
...
/** 二分法求根：要求 f(a)·f(b) < 0 */
export function bisect(f: (x: number) => number, a: number, b: number, tol = 1e-12): number {
  const fa = f(a)
  const fb = f(b)
  if (fa === 0) return a
  if (fb === 0) return b
  if (fa * fb > 0) throw new Error('二分法要求两端点函数值异号')
  let lo = a
  let hi = b
  for (let i = 0; i < 200; i++) {
    const mid = (lo + hi) / 2
...
export interface RootFindOptions {
  a?: number // 扫描区间左端（默认 -100）
  b?: number // 扫描区间右端（默认 100）
  n?: number // 扫描段数（默认 400）
  tol?: number // 根精度（默认 1e-12）
}
 ...
 */
export function findRoots(f: (x: number) => number, opts: RootFindOptions = {}): number[] {
  const a = opts.a ?? -100
  const b = opts.b ?? 100
  const n = opts.n ?? 400
  ...
  for (let i = 1; i <= n; i++) {
    ...
    if (Math.abs(y) < tol && i < n) {
      // 函数值本身接近 0（重根或根恰在采样点）
      if (roots.length === 0 || Math.abs(x - roots[roots.length - 1]!) > 1e-6) roots.push(x)
    } else if (prevY * y < 0) {
  ...
  for (const r of roots) {
    if (out.length === 0 || Math.abs(r - out[out.length - 1]!) > 1e-6) out.push(r)
  }
  return out
}
...
function gaussSolveFloat(aug: number[][], n: number): number[] | null {
  for (let col = 0; col < n; col++) {
    let pr = col
    for (let r = col + 1; r < n; r++) {
      if (Math.abs(aug[r]![col]!) > Math.abs(aug[pr]![col]!)) pr = r
    }
    if (Math.abs(aug[pr]![col]!) < 1e-14) return null
    ;[aug[col], aug[pr]] = [aug[pr]!, aug[col]!]
    const piv = aug[col]![col]!
    for (let j = col; j <= n; j++) aug[col]![j] = aug[col]![j]! / piv
    for (let r = 0; r < n; r++) {
      if (r === col) continue
      const f = aug[r]![col]!
      if (f === 0) continue
      for (let j = col; j <= n; j++) aug[r]![j] = aug[r]![j]! - f * aug[col]![j]!
    }
  }
  return aug.map((row) => row[n]!)
}

export interface MultiNewtonOptions {
  tol?: number
  maxIter?: number
}
 ...
 */
export function multiNewton(
  F: (x: number[]) => number[],
  x0: number[],
  opts: MultiNewtonOptions = {},
): number[] | null {
  const n = x0.length
  ...
  for (let iter = 0; iter < maxIter; iter++) {
    const fx = F(x)
    if (fx.every((v) => Math.abs(v) < tol)) return x
    const J: number[][] = []
    for (let i = 0; i < n; i++) {
      const row: number[] = []
      for (let j = 0; j < n; j++) {
    ...
    if (!nx.every(Number.isFinite)) return null
    if (delta.every((d) => Math.abs(d) < tol)) return nx
    x = nx
  }
  return null
}
...
export function simpson(f: (x: number) => number, a: number, b: number, n = 100): number {
  if (n % 2 !== 0) n++
  const h = (b - a) / n
  let s = f(a) + f(b)
  for (let i = 1; i < n; i++) {
...
const GL16: Array<[number, number]> = [
...
export function matchesSign(root: Expr, sign: Sign): boolean {
