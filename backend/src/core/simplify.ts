...
function piMultipleOf(e: Expr): Rat | null {
  if (e.kind === 'sym' && e.name === 'pi') return rat(1)
  if (e.kind === 'mul' && e.factors.length === 1 && e.factors[0]!.kind === 'sym' && e.factors[0]!.name === 'pi') {
    return e.coeff
  }
  // 数值参数仅在为 0 时视为 0·π（其余弧度值不做精确求值，如 sin(2)）
  if (e.kind === 'num' && isZeroRat(e.rat)) return rat(0)
  return null
}

// sin/cos/tan 在 k·π（0 ≤ k < 2）处的精确值表；值用字符串标记以便构造
type TrigVal = { sin: string; cos: string; tan: string | null }
const TRIG_TABLE: Record<string, TrigVal> = {
...
function exactFnValue(name: FnName, arg: Expr): Expr | null {
  if (name === 'exp') {
    if (arg.kind === 'num' && isZeroRat(arg.rat)) return num(1)
    return null
  }
  if (name === 'ln') {
