...
export function substitute(e: Expr, varName: string, value: Expr): Expr {
  switch (e.kind) {
    case 'num':
      return e
    case 'sym':
...
/** 线性方程：Σ coeffs[v]·v + const = 0 */
export interface LinearEq {
  coeffs: Map<string, Rat>
  const: Rat
}

/** 乘法因子的类型：常数 / 单个变量（线性项）/ 非线性 */
type ProductType =
  | { kind: 'const'; value: Rat }
  | { kind: 'linear'; varName: string; coeff: Rat }
  | null

function productType(factors: Expr[], vars: ReadonlySet<string>): ProductType {
  ...
  for (const f of factors) {
    ...
    if (f.kind === 'sym') {
      if (!vars.has(f.name)) return null // e/pi/其他符号进不了有理系数
      if (varName !== null) return null // 两个变量相乘 → 非线性
      varName = f.name
      continue
    }
    if (f.kind === 'pow') {
  ...
  if (varName === null) return { kind: 'const', value: coeff }
  return { kind: 'linear', varName, coeff }
}

/** 把 Expr 化为关于 vars 的线性式 Σcoeff·var + const = 0；非线性返回 null */
export function toLinearEq(e: Expr, vars: ReadonlySet<string>): LinearEq | null {
  const coeffs = new Map<string, Rat>()
  let cnst: Rat = RAT0
  const rec = (x: Expr): boolean => {
    switch (x.kind) {
      case 'num':
        cnst = addRat(cnst, x.rat)
        return true
      case 'sym':
        if (!vars.has(x.name)) return false
      ...
      case 'mul': {
        ...
        const c = x.coeff
        if (p.kind === 'const') cnst = addRat(cnst, mulRat(c, p.value))
        else coeffs.set(p.varName, addRat(coeffs.get(p.varName) ?? RAT0, mulRat(c, p.coeff)))
        return true
      }
      case 'pow': {
        const { base, exp } = x
...
export type LinSolveResult =
  | { kind: 'solution'; solution: Map<string, Expr> } // 变量 → 表达式（唯一解时全部为常数）
  | { kind: 'contradiction' }

/** 解线性方程组：Gaussian-Jordan 消元，精确有理数；欠定时用自由变量参数化 */
export function solveLinearSystem(eqs: LinearEq[], vars: string[]): LinSolveResult {
  if (vars.length === 0) {
    // 无变量：检查是否所有方程都是 0=0
  ...
  const m: Rat[][] = eqs.map((eq) => {
    const row: Rat[] = vars.map((v) => eq.coeffs.get(v) ?? RAT0)
    row.push(eq.const)
    return row
  })
  const rows = m.length
  ...
  for (let col = 0; col < cols && pivotRow < rows; col++) {
    ...
    for (let j = col; j <= cols; j++) m[pivotRow]![j] = mulRat(m[pivotRow]![j]!, inv)
    for (let r = 0; r < rows; r++) {
      if (r === pivotRow) continue
      const f = m[r]![col]!
      if (isZeroRat(f)) continue
      for (let j = col; j <= cols; j++) m[r]![j] = subRat(m[r]![j]!, mulRat(f, m[pivotRow]![j]!))
    }
    pivotRow++
  }
  ...
  for (let r = 0; r < rows; r++) {
    ...
    if (allZero && !isZeroRat(m[r]![cols]!)) return { kind: 'contradiction' }
  }

  const solution = new Map<string, Expr>()
  for (let r = 0; r < pivotRow; r++) {
    ...
    for (let c = pc + 1; c < cols; c++) {
      const a = m[r]![c]!
      if (isZeroRat(a) || pivotCols.has(c)) continue
      terms.push(mul(num(a.n, a.d), sym(vars[c]!)))
    }
    let expr: Expr = num(-b.n, b.d)
    for (const t of terms) expr = add(expr, negRatTerm(t))
...
function negRatTerm(t: Expr): Expr {
  if (t.kind === 'mul') return mul(num(-t.coeff.n, t.coeff.d), ...t.factors)
  return mul(num(-1), t)
}

// ---------- 方程组主入口 ----------

export type SystemSolution = Map<string, Expr>
export type SolveSystemResult =
  | { kind: 'solutions'; solutions: SystemSolution[] }
  | { kind: 'contradiction' }
  | { kind: 'identity' }
  | { kind: 'unsupported' }
 ...
 */
export function solveSystem(eqs: Expr[], vars: string[], signs?: Map<string, Sign>): SolveSystemResult {
  const varSet = new Set(vars)
  const linEqs: LinearEq[] = []
  const nonLin: Expr[] = []
  for (const eq of eqs) {
    const le = toLinearEq(eq, varSet)
    if (le !== null) {
      // 0 = 0 恒等方程跳过
      if (le.coeffs.size === 0 && isZeroRat(le.const)) continue
      linEqs.push(le)
    } else {
      nonLin.push(eq)
    }
  ...
  let lin: LinSolveResult
  if (linEqs.length === 0) {
    lin = { kind: 'solution', solution: new Map() }
  } else {
    lin = solveLinearSystem(linEqs, vars)
    if (lin.kind === 'contradiction') return { kind: 'contradiction' }
  }
  ...
  if (nonLin.length === 0) {
    ...
    for (const v of vars) out.set(v, linearSolution.get(v) ?? sym(v))
  ...
  // 线性解代入非线性方程
  const reduced: Expr[] = nonLin.map((eq) => {
    let r = eq
    for (const [v, expr] of linearSolution) r = substitute(r, v, expr)
    return r
  })
  ...
  if (freeVars.length === 1) {
    ...
    let roots: Expr[] | null = null // null 表示某方程 unsupported
    for (const eq of reduced) {
      // 恒等/矛盾检查：方程化为常数
      if (eq.kind === 'num') {
        if (isZeroRat(eq.rat)) continue
      ...
      const res: SolveResult = solvePoly(poly)
      if (res.kind !== 'root') {
        if (res.kind === 'identity') continue
        // 二次方程判别式 < 0：实数域无解，即矛盾
        if (res.kind === 'contradiction' || res.kind === 'noReal') return { kind: 'contradiction' }
    ...
    if (roots === null) return { kind: 'unsupported' }
    const solutions = roots.map((root) => {
      const out = new Map<string, Expr>()
      for (const [v, expr] of linearSolution) out.set(v, substitute(expr, target, root))
      out.set(target, root)
      return out
    })
...
function finalize(solutions: SystemSolution[], signs?: Map<string, Sign>): SolveSystemResult {
  ...
  const filtered = solutions.filter((sol) => {
    for (const [v, s] of signs) {
      ...
      if (expr !== undefined && !matchesSign(expr, s)) return false
    }
    return true
  })
...
function keyOfExpr(e: Expr): string {
  switch (e.kind) {
    case 'num':
