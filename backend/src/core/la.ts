...
import { divRat, isZeroRat, mulRat, negRat, subRat, type Rat } from './expr.js'

/** 是否方阵 */
function isSquare(m: Rat[][]): boolean {
 ...
 * 非方阵返回 null；奇异地返回 0。
 */
export function matDet(m: Rat[][]): Rat | null {
  if (m.length === 0) return { n: 1n, d: 1n } // 空矩阵行列式为 1
  if (!isSquare(m)) return null
  const n = m.length
  const a: Rat[][] = m.map((row) => [...row])
  let sign = 1n
  let prev: Rat = { n: 1n, d: 1n }
  for (let k = 0; k < n - 1; k++) {
 ...
 * 非方阵或奇异返回 null。
 */
export function matInvert(m: Rat[][]): Rat[][] | null {
  ...
  for (let col = 0; col < n; col++) {
    ...
    if (pr === -1) return null // 奇异
    ;[aug[col], aug[pr]] = [aug[pr]!, aug[col]!]
    // 归一化主元行
    const piv = aug[col]![col]!
    for (let j = col; j < 2 * n; j++) aug[col]![j] = divRat(aug[col]![j]!, piv)
    // 消去其他行的该列
    for (let r = 0; r < n; r++) {
      if (r === col) continue
      const f = aug[r]![col]!
