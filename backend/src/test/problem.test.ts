...
describe('历史保存 / 恢复', () => {
  ...
  it('reset 清空全部对象与历史', () => {
    ...
    p.addPoint('A', '1', '2', '', '')
    p.addExprEq('2a', '6')
    expect(p.getSymbolNames()).toEqual(['a'])
    expect(p.getPointNames()).toEqual(['A'])
    expect(p.getCondIds()).toEqual(['2a = 6'])
    p.reset()
    expect(p.getSymbolNames()).toEqual([])
    expect(p.getPointNames()).toEqual([])
    expect(p.getCondIds()).toEqual([])
