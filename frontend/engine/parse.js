...
import { add, fn, mul, num, pow, rat, sqrt, sym } from './expr.js';
const FN_NAMES = new Set(['sin', 'cos', 'tan', 'exp', 'ln', 'sqrt', 'acos']);
...
/** 有限小数转分数节点 */
function numFromNumber(value) {
    if (!Number.isFinite(value))
        throw new ParseError('非法数字', 0);
    if (Number.isInteger(value))
        return num(BigInt(value));
            ...
            case 'ident': {
                if (FN_NAMES.has(t.name)) {
                    if (this.peek()?.kind !== 'lparen')
                        throw new ParseError(`函数 ${t.name} 后需要括号`, t.pos);
                    this.next();
...
function requireIntExp(e, pos) {
    if (e.kind === 'num' && e.rat.d === 1n)
        return e.rat;
