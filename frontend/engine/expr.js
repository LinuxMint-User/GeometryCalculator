...
export const isZeroRat = (r) => r.n === 0n;
export const isOneRat = (r) => r.n === 1n && r.d === 1n;
export const isNegRat = (r) => r.n < 0n;
export const isIntRat = (r) => r.d === 1n;
export const negRat = (r) => ({ n: -r.n, d: r.d });
export const invRat = (r) => {
    if (r.n === 0n)
        throw new Error('零没有倒数');
    return r.n > 0n ? { n: r.d, d: r.n } : { n: -r.d, d: -r.n };
};
export const addRat = (a, b) => rat(a.n * b.d + b.n * a.d, a.d * b.d);
export const subRat = (a, b) => addRat(a, negRat(b));
export const mulRat = (a, b) => rat(a.n * b.n, a.d * b.d);
export const divRat = (a, b) => mulRat(a, invRat(b));
/** 整数次幂（BigInt），指数非负时直接算，负指数由调用方转为倒数 */
export function powRat(base, e) {
    if (e < 0n)
        throw new Error('powRat 只接受非负指数');
    if (e === 0n)
        return 1n;
    let result = 1n;
    let b = base;
...
export function compareRat(a, b) {
    const l = a.n * b.d;
    const r = b.n * a.d;
...
export function extractSquare(n) {
    let k = 1n;
    let m = n;
    for (let p = 2n; p * p <= m;) {
        if (m % p === 0n) {
...
// ---------- 常量 ----------
export const ZERO = { kind: 'num', rat: { n: 0n, d: 1n } };
export const ONE = { kind: 'num', rat: { n: 1n, d: 1n } };
export const NEG_ONE = { kind: 'num', rat: { n: -1n, d: 1n } };
// ---------- 工厂 ----------
/** 有理数节点 */
...
/** 符号节点，可带取值范围假设 */
export function sym(name, sign = null) {
    return { kind: 'sym', name, sign };
}
export function fn(name, arg) {
...
export function sqrt(arg) {
    if (arg.kind === 'num') {
        const { n, d } = arg.rat;
        if (n < 0n)
            return { kind: 'sqrt', arg }; // 实数域无定义，保留
        if (n === 0n)
            return ZERO;
        ...
        if (m === 1n)
            return num(k, d);
        const coeff = rat(k, d);
        const radical = { kind: 'sqrt', arg: num(m) };
        return isOneRat(coeff) ? radical : mul(num(coeff.n, coeff.d), radical);
    }
    return { kind: 'sqrt', arg };
}
/** 负号 */
export function neg(e) {
    return mul(NEG_ONE, e);
}
function splitCoeff(e) {
    if (e.kind === 'num')
        return { coeff: e.rat, rest: ONE };
    if (e.kind === 'mul')
        return { coeff: e.coeff, rest: mulFromFactors(e.factors) };
    return { coeff: { n: 1n, d: 1n }, rest: e };
}
function mulFromFactors(factors) {
    return factors.length === 1 ? factors[0] : { kind: 'mul', coeff: { n: 1n, d: 1n }, factors };
}
function withCoeff(coeff, rest) {
    if (isOneRat(coeff))
        return rest;
    if (rest.kind === 'mul')
        return { kind: 'mul', coeff, factors: rest.factors };
...
export function add(...items) {
    ...
    for (const e of flat) {
        ...
        if (g !== undefined)
            g.coeff = addRat(g.coeff, coeff);
        else
            groups.set(key, { coeff, rest });
    }
    const terms = [];
    ...
    terms.sort(byKey);
    if (!isZeroRat(constPart))
        terms.push(num(constPart.n, constPart.d));
    if (terms.length === 0)
        return ZERO;
    if (terms.length === 1)
        return terms[0];
...
export function mul(...items) {
    ...
    for (const f of rawFactors) {
        ...
        if (g !== undefined)
            g.exp = addRat(g.exp, exp);
        else
            byBase.set(key, { base, exp });
    }
    const factors = [];
...
export function pow(base, exp) {
    if (exp.d !== 1n)
        throw new Error(`幂的指数必须为整数，收到 ${exp.n}/${exp.d}`);
    if (isZeroRat(exp))
        return ONE;
    if (isOneRat(exp))
        return base;
    if (base.kind === 'num') {
        if (base.rat.n === 0n && exp.n < 0n)
            throw new Error('零的负次幂无定义');
        const e = exp.n;
        if (e >= 0n)
            return num(powRat(base.rat.n, e), powRat(base.rat.d, e));
